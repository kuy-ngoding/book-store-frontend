...
} from "../../../modules/ministry/api/child-dedication.api";
import { ChildDedication } from "../../../modules/ministry/entities/child-dedication.entity";

function ChildDedicationList() {
  const [sidebarOpen, setSidebarOpen] = useState(false);
  const [selectedItems, setSelectedItems] = useState([]);
  const [childdedicationList, setChildDedicationList] = useState([]);
  ...
  const { data, refetch } = useGetAllChildDedicationQuery(
    {
      page: currentPage,
      limit: pageSize,
      ...filterParams,
  ...
  const [deleteChildDedication] = useDeleteChildDedicationMutation();

  useEffect(() => {
    if (data) {
      setChildDedicationList(data.data);
  ...
  const handleDelete = async (id: string) => {
    if (
      window.confirm("Are you sure you want to delete this childdedication?")
    ) {
      await deleteChildDedication({ id }).unwrap();
      refetch();
    }
  };
  ...
  const childdedicationColumns = React.useMemo<Column<ChildDedication>[]>(
    () =>
      [
        {
          Header: "Title",
        ...
        {
          Header: "ChildDedication Date",
          accessor: "childdedicationDate",
          Cell: (row: any) => {
          ...
          Cell: ({ row }) => (
            <div>
              <Link
                ...
                aria-label="detail"
                to={`/childdedication-management/childdedications/detail/${row.original._id}`}
              >
                <FaEye />
              </Link>
              <Link
                ...
                aria-label="edit"
                to={`/childdedication-management/childdedications/edit/${row.original._id}`}
              >
                <FaEdit />
              </Link>
              <button
        ...
        <main>
          <div className="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
            ...
            <div className="bg-white shadow-lg rounded-sm border border-gray-200 relative">
              ...
              <ReactTable
                tableOptions={{
                  columns: childdedicationColumns,
                  data: childdedicationList,
                  manualFilters: true,
...
export default ChildDedicationList;
